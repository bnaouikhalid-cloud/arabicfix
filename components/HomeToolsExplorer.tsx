...
export const HomeToolsExplorer: React.FC<HomeToolsExplorerProps> = ({ tools }) => {
  ...
  const filteredTools = useMemo(() => {
    return tools.filter((tool) => {
      ...
      const isSearchReversed = (q.includes('مقلوب') || q.includes('مكعوس') || q.includes('pdf')) && tool.slug === 'fix-reversed-arabic';
    ...
    <>
      ...
      <div className="relative max-w-2xl mx-auto mb-6">
        <div className="relative flex items-center">
          <div
            className={`absolute top-1/2 -translate-y-1/2 text-[var(--muted-foreground)] pointer-events-none ${
              isAr ? 'right-4' : 'left-4'
            }`}
          >
          ...
          <input
            ...
            className={`w-full py-3.5 sm:py-4 rounded-xl bg-[var(--card-bg)] border border-[var(--border)] text-[var(--foreground)] placeholder:text-[var(--muted-foreground)] focus:ring-2 focus:ring-[var(--primary)] focus:border-[var(--primary)] shadow-sm outline-none text-base sm:text-lg transition-all ${
          ...
          {searchQuery && (
            <button
              ...
              className={`absolute top-1/2 -translate-y-1/2 text-[var(--muted-foreground)] hover:text-[var(--foreground)] cursor-pointer ${
                isAr ? 'left-4' : 'right-4'
              }`}
            >
              <X className="w-4 h-4" />
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 mt-4">
        <span className="text-xs font-bold text-[var(--secondary-foreground)] uppercase tracking-wider">
        ...
        {popularShortcuts.map((item) => (
          <Link
            ...
            className="px-3 py-1 bg-[var(--badge-bg)] text-[var(--badge-text)] rounded-full text-xs font-semibold transition-colors hover:bg-[var(--badge-bg)]/80"
          >
            {item.label[lang]}
          </Link>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start mt-12 sm:mt-16">
        ...
        <aside className="lg:col-span-3 space-y-2">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-xs font-bold text-[var(--secondary-foreground)] uppercase tracking-widest">
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between p-3 rounded-xl text-sm font-semibold transition-all cursor-pointer ${
                    isSelected
                      ...
                      : 'bg-[var(--card-bg)] border border-[var(--border)] text-[var(--foreground)] hover:border-[var(--primary)]/20'
                  }`}
                >
                  <span className="truncate">{cat.name[lang]}</span>
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-bold ${
                      isSelected ? 'bg-[var(--primary)]/20' : 'bg-[var(--badge-bg)]'
                    }`}
                  >
                    {count}
                  </span>
          ...
          <div className="mt-6 p-4 bg-[var(--primary)] text-[var(--primary-foreground)] rounded-2xl shadow-lg relative overflow-hidden">
            <div className="relative z-10">
              <p className="text-xs font-bold uppercase opacity-80 mb-1 tracking-wider">
                {isAr ? 'الخصوصية أولاً' : 'Privacy First'}
              </p>
              <p className="text-sm leading-snug font-medium text-[var(--primary-foreground)]/90">
                {isAr
                  ? 'لا يتم حفظ أي بيانات. جميع المعالجات تجري محلياً داخل متصفحك.'
                  : 'No data is stored. All processing happens in your browser.'}
              </p>
            </div>
            <div className="absolute -right-4 -bottom-4 w-20 h-20 bg-[var(--primary-foreground)]/10 rounded-full blur-xl pointer-events-none"></div>
          </div>
        </aside>
        ...
        <div className="lg:col-span-9 space-y-4">
          ...
          {filteredTools.length === 0 ? (
            <div className="text-center py-16 bg-[var(--card-bg)] border border-[var(--border)] rounded-2xl p-8 shadow-xs">
              ...
              <button
                ...
                className="text-xs font-bold text-[var(--primary)] hover:underline underline-offset-4 cursor-pointer"
              >
